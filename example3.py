...
model.add(Dense(10,activation='relu')) # (第一层隐藏层)输入层到隐藏层，10表示该隐藏层神经元的数量，activation表示激活函数
...
# 训练模型，epochs表示训练轮数，batch_size表示每次训练的样本数量，verbose表示训练过程中控制台是否显示信息，把测试集当作验证集验证是否过拟合
# 过拟合：验证集和训练集的损失函数值相差较大，训练集(平时练习)好，但验证集(月考)差
history = model.fit(x_train, y_train_one, epochs=100, batch_size=15, verbose=1,validation_data=(x_test, y_test_one))
